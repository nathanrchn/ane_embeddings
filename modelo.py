...
class Attention(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.q_proj = nn.Linear(hidden_size, hidden_size, bias=True, dtype=torch.float16)
        self.k_proj = nn.Linear(hidden_size, hidden_size, bias=True, dtype=torch.float16)
        self.v_proj = nn.Linear(hidden_size, hidden_size, bias=True, dtype=torch.float16)
        self.o_proj = nn.Linear(hidden_size, hidden_size, bias=True, dtype=torch.float16)
    ...
    def forward(self, hidden_state: Tensor, mask: Tensor) -> Tensor:
        ...
        att = (query_state @ key_state.transpose(-2, -1)) * head_dim ** -0.5
        att = F.softmax(att + mask, dim=-1)
        att = att @ value_state

        att = att.transpose(1, 2).contiguous().view(batch_size, seq_len, hidden_size).to(torch.float16)
        return self.o_proj(att)
    
class MLP(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.fc1 = nn.Linear(hidden_size, intermediate_size, dtype=torch.float16)
        self.fc2 = nn.Linear(intermediate_size, hidden_size, dtype=torch.float16)
...
class Model(nn.Module):
    ...
    def forward(self, input_ids: Tensor, mask: Tensor) -> Tensor:
        ...
        return F.normalize(hidden_states[:, 0].to(torch.float32), p=2, dim=1)
